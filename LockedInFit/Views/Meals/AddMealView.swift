    ...
    var body: some View {
        NavigationStack {
            Form {
                ...
                Section {
                    Button {
                        ...
                        Label("Add preset foods", systemImage: "cart.badge.plus")
                    }
                } header: {
                    Text("From presets")
                } footer: {
                    Text("Pick as many saved foods as the meal needs, each with its own amount, then add the whole cart at once.")
                }
            ...
            .sheet(isPresented: $showPresetPicker) {
                // The picker hands back a whole cart, not one food at a time:
                // see PresetCartLine.makeFoodItem for why each line's grams
                // and scaled nutrition are built together. Order continues
                // from whatever's already in addedItems, so carting foods
                // after a photo/speech/manual pass appends instead of
                // reshuffling.
                PresetPickerView { cartLines in
                    addedItems.append(contentsOf:
                        PresetCartMath.foodItems(for: cartLines, startingAt: addedItems.count))
                    recalcTotals()
                }
            }
        }
