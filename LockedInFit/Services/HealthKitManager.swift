...
final class HealthKitManager {
    ...
    /// writes are the one SwiftData pattern this app has actually seen
    /// deadlock in the wild, so all writes stay on the main context; the
    /// PerfLog line below proves the import costs single-digit
    /// milliseconds (the data volume is small and `syncRecent` only spans
    /// 2 days), which never justified a second write context. What made
    /// HealthKit expensive originally was the 1-second timer, fixed
    /// separately. Reentrancy guard because the observer queries fire once
    /// per incoming batch of Health samples and used to stack import
    /// passes.
    @MainActor
    func sync(days: Int? = 60, context: ModelContext) async {
        ...
        do {
            ...
            let imported = PerfLog.measure("healthkit.import") {
    ...
    private func importAll(steps: [(Date, Int)], activeEnergy: [(Date, Double)],
                           weights: [HKSampleValue], bodyFats: [HKSampleValue],
    ...
    @MainActor
    private func upsertDailyHealthWeights(_ samples: [HKSampleValue], existing: [BodyWeightEntry], context: ModelContext) -> Int {
        var changes = 0
    ...
    @MainActor
    private func upsertDailyHealthBodyFat(_ samples: [HKSampleValue], existing: [BodyFatEntry], context: ModelContext) -> Int {
        var changes = 0
        var existingByDay = latestExistingByDay(existing, context: context, changes: &changes)
        let samplesByDay = latestSamplesByDay(samples)
...
/// A HealthKit sample reduced to what the importers need.
